...
void LSM303::calibrate(vector<Tb>* min , vector<Tb>* max){
	Wire.begin();
	compass.init();
	...
	for( uint32_t start = millis(); (millis()-start) < time;){
		compass.read();
		min->x = min(running_min.x , compass.m.x);
		min->y = min(running_min.y , compass.m.y);
		min->z = min(running_min.z , compass.m.z);

		max->x = max(running_max.x , compass.m.x);
		max->y = max(running_max.y , compass.m.y);
		max->z = max(running_max.z , compass.m.z);
